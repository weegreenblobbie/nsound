...
//~    Buffer freq_offset = _sine->gaussianNoise(weights.size(), 0.0, std::min(frequency / 100.0,  2.0));
...
void Nsound::pipeorgan_normalize()
{
    auto lut = LookupTable();
    //           Freq (Hz)    dB            ISO 226 @40 phon
    lut.add_point(   20.0,    -59.9);
    lut.add_point(   25.0,    -53.9);
    lut.add_point(   32.0,    -48.2);
    lut.add_point(   40.0,    -42.6);
    lut.add_point(   50.0,    -37.8);
    lut.add_point(   63.0,    -33.1);
    lut.add_point(   80.0,    -28.5);
    lut.add_point(  100.0,    -24.4);
    lut.add_point(  125.0,    -20.6);
    lut.add_point(  160.0,    -16.7);
    lut.add_point(  200.0,    -13.4);
    lut.add_point(  250.0,    -10.4);
    lut.add_point(  315.0,     -7.6);
    lut.add_point(  400.0,     -5.0);
    lut.add_point(  500.0,     -3.1);
    lut.add_point(  630.0,     -1.3);
    lut.add_point(  800.0,     -0.1);
    lut.add_point( 1000.0,      0.0);
    lut.add_point( 1250.0,     -1.8);
    lut.add_point( 1600.0,     -2.5);
    lut.add_point( 2000.0,      0.8);
    lut.add_point( 2500.0,      3.5);
    lut.add_point( 3150.0,      4.4);
    lut.add_point( 4000.0,      3.4);
    lut.add_point( 5000.0,      0.0);
    lut.add_point( 6300.0,     -5.8);
    lut.add_point( 8000.0,    -11.8);
    lut.add_point(10000.0,    -14.3);
    lut.add_point(12500.0,    -11.5);
    lut.add_point(16000.0,    -12.0);
    lut.add_point(20000.0,    -52.8);

    const float64 sr = 48000.0;

    auto organ = OrganPipe(sr);

    Buffer weights;

    std::map<float64, float64> scores;

    for (const auto & pair : principal_stops)
    {
        const auto & freq = pair.first;
        auto signal = organ.play(0.100, freq)[0];

        const auto spec = Spectrogram(signal, sr, 0.060, 0.010, HANNING);

        const auto mag = spec.getMagnitude()[0];

        if (weights.empty())
        {
            // Weights that map the ISO 226 @40 phon weights to the spectrogram's
            // frequency acces.
            for (auto freq : spec.getFrequencyAxis()) weights << std::pow(10.0, lut.interpolate(freq) / 10.0);
        }

        auto score = (mag * weights).getSum();
    ...
    std::cout << "---------------------------------\nNormalized Scores\n";
    auto middle_c4 = scores[278.42000];
    for (const auto & pair : scores)
    {
        std::cout << pair.first << " = " << (1.0 / (pair.second / middle_c4)) << "\n";
    }
